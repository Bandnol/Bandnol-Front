...
- 해결방안: 가짜 데이터 코드를 삭제하고 useEffect에서 fetchReplycomment만 호출하도록 변경하고 reply.content와 reply.nickname을 모달에 저장해서 듸움

화면에 노출되지 않는 이미지를 viewshot으로 가져와야 하는 문제
- 원인 분석: 투명도를 100으로 하는 방법 → 캡쳐가 투명하게 됨. 모달을 0.01초동안 띄우고 그동안 캡쳐하는 방법 → 안정적이지 않음.
- 해결 방안: 모달의 좌표를 -1000, -1000만큼 이동해 보이지 않는 곳에서 모달을 보여주고 캡쳐함.
